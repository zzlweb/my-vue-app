...
export const darkPalette = (originColor: string, i: number, format: string) => {    
    // 将 color 转换为 hsv    
    const lightColor = Color(lightPalette(originColor, 10 - i + 1, format));    
    ...
    const baseSaturation: number = baseColor.saturationv();    
    const step: number = Math.ceil((baseSaturation - 9) / 4);    
    const step1to5: number = Math.ceil((100 - baseSaturation) / 5);    
